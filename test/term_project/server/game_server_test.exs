defmodule TermProject.Server.GameServerTest do
  ...
  test "joining a second user to a public lobby" do
    ...
    {:ok, {same_lobby_id, updated_lobby_state}} =
      TermProject.Server.GameServer.request_lobby(username2)
  ...
  test "creates a new private lobby with a password" do
    ...
    {:ok, {lobby_id, lobby_state}} =
      TermProject.Server.GameServer.request_lobby(username, password)
  ...
  test "joining a second user to an existing private lobby" do
    ...
    {:ok, {lobby_id, lobby_state}} =
      TermProject.Server.GameServer.request_lobby(username1, password)
    ...
    {:ok, {^lobby_id, updated_lobby_state}} =
      TermProject.Server.GameServer.request_lobby(username2, password)
  ...
  test "error returned if private lobby is full" do
    ...
    assert {:error, :lobby_full} =
             TermProject.Server.GameServer.request_lobby(username3, password)
  end
end
